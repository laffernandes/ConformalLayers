...
class ConformalLayers(torch.nn.Module):
    ...
    def forward(self, input: DenseTensor) -> DenseTensor:
        ...
        if self.training:
            ...
            # Compute the homogeneous coordinate of the input vectors
            in_channels = in_dims[0] if input.dim() > 2 else 1
            input_extra = torch.linalg.norm(input.view(batches, in_channels, -1), ord=2, dim=2)
            alpha_upper = torch.ones((in_channels,), dtype=input.dtype, device=input.device)
            (output, output_extra), _ = self._modulez(((input, input_extra), alpha_upper))
            # Apply the modules as is
            output = output / output_extra.view(batches, -1, *map(lambda _: 1, range(2, output.dim())))
        else:
            with torch.no_grad():
